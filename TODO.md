...
- [ ] **GAES の ewidth 決定に PDOS を読む**（2026-09-26、ユーザー指示）。いまは total DOS（原子あたりに直した値）だけで判定する。希薄な成分（例 La0.999Ge0.001 の Ge）の semicore は total DOS には濃度分（~1e-3）しか現れず閾値と同程度なので検知できない。成分ごとの PDOS（`dos` の `pdos` 出力、濃度は掛かっていない）を判定に加え、全曲線の AND でギャップ区間を決める（仕様 AkaiKKRPythonUtil/docs/ewidth_tuning_scheme.md §0.3 に設計方針あり）。実装は pyakaikkr.gaes 側（`gap.py` の区間検出を「mesh + 任意本数の DOS 曲線」に一般化）と AkaikkrGaesWorkChain（pdos を渡す）の両方。
